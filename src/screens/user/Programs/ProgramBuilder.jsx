...
export default function ProgramBuilder({ route, navigation }) {
    ...
    try {
      ...
      if (res?.success) {
        ...
        if (res.data.chapters?.length > 0) {
          await fetchChapterContent(res.data.chapters[0].chapter_id);
        }
        setIsEditing(false);
      } else {
        Toast.show({ type: "error", text1: res?.message || "Failed to fetch tree" });
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Slide-over Structure Drawer Sheet */}
      <Modal
...
const styles = StyleSheet.create({
  ...
  navBtn: { padding: 4 },
  headerTitle: { fontSize: 16, fontWeight: "600", color: "#000", flex: 1, marginHorizontal: 12 },
  menuBtn: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#eff6ff", paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
  menuBtnText: { color: "#2563eb", fontWeight: "600", fontSize: 13 },
  editorContainer: { flex: 1 },
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "flex-end" },
  sidebarSheet: { height: "80%", backgroundColor: "#fff", borderTopLeftRadius: 16, borderTopRightRadius: 16, padding: 16 },
