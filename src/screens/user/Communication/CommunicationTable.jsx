...
export default function CommunicationTable() {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Failed to fetch templates',
      });
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (res?.success) {
        Toast.show({
          type: 'success',
          text1: 'Success',
          text2: res.message || 'Template deleted successfully',
        });
        fetchTemplates();
      } else {
        Toast.show({
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: e?.response?.data?.message || 'Server error while deleting',
      });
    } finally {
      setLoading(false);
      setSelectedId(null);
  ...
  const columns = [
    ...
    },
    {
      ...
      center: true,
      width: 110,
      cell: row => <Text style={styles.cellText}>{row.email_count || 0}</Text>,
    },
    {
      name: 'Opened',
      center: true,
  ...
  return (
    <View style={styles.screenContainer}>
      ...
      {/* REUSABLE STATUS CONFIRM MODAL FOR DELETE */}
      <StatusConfirmModal
